...
local function getReloadTimingFromParam(param)
    ...
    local reload_xmag4_feed = param.reload_xmag4_feed * 1000
    local reload_xmag4_cooldown = param.reload_xmag4_cooldown * 1000
    local empty_xmag4_feed = param.empty_xmag4_feed * 1000
    local empty_xmag4_cooldown = param.empty_xmag4_cooldown * 1000
    ...
    if (reload_xmag4_feed == nil or reload_xmag4_cooldown == nil or empty_xmag4_feed == nil or empty_xmag4_cooldown == nil) then
        return nil
    end
    -- 顺序返回获取到的这 12 个参数
    return reload_feed, reload_cooldown, empty_feed, empty_cooldown, reload_xmag2_feed, reload_xmag2_cooldown, empty_xmag2_feed, empty_xmag2_cooldown, reload_xmag3_feed, reload_xmag3_cooldown, empty_xmag3_feed, empty_xmag3_cooldown, reload_xmag4_feed, reload_xmag4_cooldown, empty_xmag4_feed, empty_xmag4_cooldown
end
...
function M.tick_reload(api)
    ...
    local reload_feed, reload_cooldown, empty_feed, empty_cooldown, reload_xmag2_feed, reload_xmag2_cooldown, empty_xmag2_feed, empty_xmag2_cooldown, reload_xmag3_feed, reload_xmag3_cooldown, empty_xmag3_feed, empty_xmag3_cooldown, reload_xmag4_feed, reload_xmag4_cooldown, empty_xmag4_feed, empty_xmag4_cooldown = getReloadTimingFromParam(param)
    ...
    elseif (api:getMagExtentLevel() == 2) then
        ...
        elseif (api:getMagExtentLevel() == 3) then
            if (cache.is_tactical) then
                if (reload_time < reload_xmag4_feed) then
                    return TACTICAL_RELOAD_FEEDING, reload_xmag4_feed - reload_time
                elseif (reload_time >= reload_xmag4_feed and reload_time < reload_xmag4_cooldown) then
                    if (cache.reloaded ~= 1) then
                        if (api:isReloadingNeedConsumeAmmo()) then
                            api:putAmmoInMagazine(api:consumeAmmoFromPlayer(cache.needed_count))
                        else
                    ...
                    return TACTICAL_RELOAD_FINISHING, reload_xmag4_cooldown - reload_time
                else
                    return NOT_RELOADING, -1
                end
            else
                if (reload_time < empty_xmag4_feed) then
                    return EMPTY_RELOAD_FEEDING, empty_xmag4_feed - reload_time
                elseif (reload_time >= empty_xmag4_feed and reload_time < empty_xmag4_cooldown) then
                    if (cache.reloaded ~= 1) then
                        if (api:isReloadingNeedConsumeAmmo()) then
                            api:putAmmoInMagazine(api:consumeAmmoFromPlayer(cache.needed_count) - 1)
                        else
                    ...
                    return EMPTY_RELOAD_FINISHING, empty_xmag4_cooldown - reload_time
                else
                    return NOT_RELOADING, -1
                end
            end
