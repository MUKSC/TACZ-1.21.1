...
public final class SoundLoader {
    ...
    public static boolean load(ZipFile zipFile, String zipPath) {
        ...
        if (matcher.find()) {
            ...
            try (InputStream zipEntryStream = zipFile.getInputStream(entry); AudioInputStream audioStream = AudioSystem.getAudioInputStream(zipEntryStream)) {
                ByteBuffer bytebuffer = ByteBuffer.wrap(audioStream.readAllBytes());
    ...
    public static void load(File root) {
        ...
        if (Files.isDirectory(filePath)) {
            TacPathVisitor visitor = new TacPathVisitor(filePath.toFile(), root.getName(), ".ogg", (id, file) -> {
                try (InputStream stream = Files.newInputStream(file); AudioInputStream audioStream = AudioSystem.getAudioInputStream(stream)) {
                    ByteBuffer bytebuffer = ByteBuffer.wrap(audioStream.readAllBytes());
